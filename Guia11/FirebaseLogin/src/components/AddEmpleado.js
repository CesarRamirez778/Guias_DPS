...
export default function AddEmpleado(props) {
  ...
  const onSubmit = () => {
    let errors = {};
    if (!formData.name || !formData.lastname || !formData.dateContract) {
      ...
      db.collection(user.uid)
        ...
        .catch(() => {
          setFormError({ name: true, lastname: true, dateContract: true });
        });
    }
    setFormError(errors);
  };
    ...
    <>
      <View style={styles.container}>
        ...
        <TouchableOpacity onPress={onSubmit}>
          <Text style={styles.addButton}>Crear Empleado</Text>
        </TouchableOpacity>
      </View>
      <DateTimePickerModal
